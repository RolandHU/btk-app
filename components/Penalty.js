...
export default function Penalty({ data }) {
  ...
  const filter = /[a-zA-ZáÁéíÍÉóÓöÖőŐúÚüÜűŰ0-9\\/\[\]%]+/gm
  ...
  return (
    <div className="border border-slate-700 rounded-md overflow-hidden text-lg">
      <div className="flex flex-col md:flex-row bg-slate-800">
        <div className="flex flex-col md:flex-row flex-1 justify-between pl-10 py-5" onClick={() => setState(!state)}>
          <h2 className="pr-10 text-xl font-semibold">{data.Name}</h2>
          <p className="pt-2 md:pr-10 md:pt-0 text-slate-600">{data.ID}</p>          
        </div>
        <div className="flex font-semibold">
          <button className="w-full md:w-14 h-full py-3 md:py-0 border-x border-slate-600 bg-slate-700 hover:bg-slate-600">+</button>
          <button className="w-full md:w-14 h-full py-3 md:py-0 bg-slate-700 hover:bg-slate-600">-</button>
        </div>        
      </div>
      <div className={`${state ? "block" : "hidden"} px-10 py-5 border-t border-slate-700`} onClick={() => setState(!state)}>

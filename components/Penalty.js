...
function Penalty ({ data, func }) {
  ...
  return (
    <details id={data.Paragraph} className={`w-full ${func.penalties.includes(data) ? "border-2 border-cyan-500" : "border border-slate-700"} rounded-md text-lg overflow-clip`} open={func.penaltyState.includes(data.Paragraph)}>
      <summary className="flex flex-col md:flex-row list-none focus:border-2 focus:border-cyan-500 focus:outline-none bg-slate-800">
        ...
        <button className="w-full md:w-14 py-3 border-y md:border-y-0 md:border-x border-slate-600 bg-slate-700 hover:bg-slate-600 focus:border-2 focus:border-cyan-500 focus:outline-none" aria-label="add-one-penalty" onClick={() => func.addPenalty(data)}><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-4 h-4 m-auto"><path fillRule="evenodd" d="M12 3.75a.75.75 0 01.75.75v6.75h6.75a.75.75 0 010 1.5h-6.75v6.75a.75.75 0 01-1.5 0v-6.75H4.5a.75.75 0 010-1.5h6.75V4.5a.75.75 0 01.75-.75z" clipRule="evenodd" /></svg></button>
        <button className="w-full md:w-14 py-3 bg-slate-700 hover:bg-slate-600 focus:border-2 focus:border-cyan-500 focus:outline-none" aria-label="remove-one-penalty" onClick={() => func.removePenalty(data)}><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-4 h-4 m-auto"><path fillRule="evenodd" d="M3.75 12a.75.75 0 01.75-.75h15a.75.75 0 010 1.5h-15a.75.75 0 01-.75-.75z" clipRule="evenodd" /></svg></button>
      </summary>
